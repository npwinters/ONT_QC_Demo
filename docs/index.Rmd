...
Data were collected from the Integrated Microbial Genomes and Microbiomes (IMG) repository...
<br>

Data were initially pre-processed using the code in the chunks below. If not otherwise mentioned, all programs are open-source and publicly available. They were installed in their own environment using the conda package manager. An example of this can be found on the appendix.
...
* __QUALOFFSET__:	Offset of sequence's first quality within the FASTQ file
...
filtered_fai_files = list.files("filtered/", 
                                pattern = "NIFP2400*", full.names = TRUE)
unfiltered_fai_files = list.files("non-filtered/2024_03_26/", 
                                  pattern = "*.fasta.datetime.fai", full.names = TRUE)
```
<br>
...
import_faidx <- function(file_list, filtered=TRUE){
  ...
  for(f in seq_along(file_list)){
    ...
    if(filtered==TRUE){
      nm = gsub("_filt.fasta.fai", "", basename(file_list[[f]]))
      tmp_df$SAMPLE=nm
      tmp_df$FILT="filtered"
    } else if(filtered==FALSE){
      nm = gsub(".fasta.datetime.fai", "", basename(file_list[[f]]))
      tmp_df$SAMPLE=nm
  ...
  if(filtered==TRUE){
    names(output_list) = gsub("_filt.datetime.fasta.fai", "", basename(file_list))
  } else{
    names(output_list) = gsub(".fasta.datetime.fai", "", basename(file_list))
  }
